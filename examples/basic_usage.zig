...
pub fn main() !void {
    ...
    const root = try zzz.parse(std.testing.allocator, kobold);
    defer root.deinit();
    ...
    print("Kobold's dexterity: {}\n", .{
        root.findNth(0, .{.String = "stats"}).?.findNth(0, .{.String = "dex"}).?.getChild(0).?.value.Int
    });

    // Grab the Kobold's first ability under "abilities" -> [first, second].
    const ability = root.findNth(0, .{.String = "abilities"}).?.getChild(0).?;
